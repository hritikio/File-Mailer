...
router.post("/send", upload.array("files",5), async(req,res)=>{
    ...
    try{
        ...
        const logs=files.map((file)=>({  //map the object in ()
            email,
...
router.get("/5logs",async(req,res)=>{
    try{
        const recent_log = await log.find().sort({createdAt:-1}).limit(5).lean();
...
router.get("/alllogs",async (req,res)=>{
     try{
        const allLogs=await log.find().sort({createdAt:-1}).lean();
